...
function transmit(request, response, rule) {
  ...
  const project = rule.project
  const PRJ_REG = /\/(\w+)\.js/
  const result = PRJ_REG.exec(requestURL.path)
  const project_name = result.length > 0 ? result[1] : null

  if (
    URL_REG.test(requestURL.path)
    && requestURL.path.indexOf(project) > -1
  ) {
    utils.info(`${project_name} will proxy on local file.`)
    readLocalFiles(request, response, rule)
  } else {
    utils.tip(`Oops. The ${project_name}.js match miss. Transmit online.`)
